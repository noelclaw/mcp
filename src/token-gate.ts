...
export function tokenGateError(toolName: string): ToolResult {
  return {
    content: [{
      type: "text",
      text: [
        "🔒 **Premium Tool — NoelClaw Token Required**",
        "",
        `\`${toolName}\` is available to NoelClaw token holders.`,
        "",
        "**How to unlock:**",
        "1. Get NOELCLAW on Base  —  CA: `0x4B524015D54a27d4472F5c59c570730D69499Ba3`",
        "2. Hold at least 1 NOELCLAW in your local wallet",
        "3. Access unlocks automatically on the next tool call",
        "",
        "Run `noel_status` to see your current tier and wallet address.",
      ].join("\n"),
    }],
    isError: true,
  };
